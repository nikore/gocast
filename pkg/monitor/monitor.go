...
func NewMonitor(config *c.Config) *MonitorMgr {
	ctrl, err := bgp.NewController(config.Bgp)
	...
	mon := &MonitorMgr{
		...
		localIP:  net.ParseIP(config.Bgp.LocalIP),
	}
	...
	for _, a := range config.Apps {
		app, err := controller.NewApp(a.Name, a.Vip, a.VipConfig, a.Monitors, "config")
		if err != nil {
...
func (m *MonitorMgr) Remove(appName string) {
	...
	if a, ok := m.monitors[appName]; ok {
		...
		for _, nat := range a.app.Nats {
			...
			switch len(parts) {
			case 3:
				if err := natRule("D", a.app.Vip.Net.IP, m.localIP, parts[0], parts[1], parts[2]); err != nil {
					glog.Errorf("Failed to remove app: %s: %v", a.app.Name, err)
				}
			case 2:
				if err := natRule("D", a.app.Vip.Net.IP, m.localIP, parts[0], parts[1], parts[1]); err != nil {
					glog.Errorf("Failed to remove app: %s: %v", a.app.Name, err)
				}
			default:
				continue
	...
	if m.runMonitors(app) {
		...
		if !am.announced {
			...
			for _, nat := range app.Nats {
				...
				switch len(parts) {
				case 3:
					if err := natRule("A", app.Vip.Net.IP, m.localIP, parts[0], parts[1], parts[2]); err != nil {
						return err
					}
				case 2:
					if err := natRule("A", app.Vip.Net.IP, m.localIP, parts[0], parts[1], parts[1]); err != nil {
						return err
					}
				default:
					continue
...
func (m *MonitorMgr) CloseAll() {
	...
	for _, am := range m.monitors {
		...
		for _, nat := range am.app.Nats {
			...
			switch len(parts) {
			case 3:
				natRule("D", am.app.Vip.Net.IP, m.localIP, parts[0], parts[1], parts[2])
			case 2:
				natRule("D", am.app.Vip.Net.IP, m.localIP, parts[0], parts[1], parts[1])
			default:
				continue
			}
		}
